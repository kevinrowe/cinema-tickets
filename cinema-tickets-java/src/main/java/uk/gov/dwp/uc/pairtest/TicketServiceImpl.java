...
public class TicketServiceImpl implements TicketService {
	...
	public TicketServiceImpl(CinemaTicketFactory cinemaTicketFactory, 
	...
	private void validateNoOfTickets(TicketTypeRequest... ticketTypeRequests) {
		boolean numberOfTicketsIsInvalid = Arrays.stream(ticketTypeRequests)
    		.anyMatch(r -> r.getNoOfTickets() < 0);
	...
	private void validateMaxTicketCount(TicketTypeRequest... ticketTypeRequests) {
		var count = Arrays.stream(ticketTypeRequests)
    		.mapToInt(TicketTypeRequest::getNoOfTickets)
    		.sum();
		
		if (count > _maximumNumberOfTickets) {
			throw new InvalidPurchaseException(String.format("You cannot purchase more than %s tickets", _maximumNumberOfTickets));
		}
	}
